		...
		const app = {
			...
			general_js: {
				init: function init(){
					...
					$(".cta.cta-prep-form").on("click", function(e){
						e.preventDefault();
 						$('html, body').animate({ scrollTop: $(".prep-form").position().top }, 'slow');
					...
					$(".form-explain a").on("click", function(e){
						e.preventDefault();
 						$('html, body').animate({ scrollTop: $(".half-half-section").position().top  }, 'slow');
					}) 
					...
					let ref= $("header").attr("data-referrer") == "empty" ? "direct" : $("header").attr("data-referrer")
					let payload = { method: 'log_visitor', data: {referrer :ref, page : $(".page-main-content").attr("data-page") } }
					app.db_req.send(payload, false).then((response) =>{ 
						if(response.success){
				...
				init: function init(){
					...
					$(window).bind("load", function() {  
						setTimeout(function(){  
							...
							data_8.src = "https://webservices.data-8.co.uk/javascript/loader.ashx?key=K3UB-54SM-JD5E-EX5P&load=InternationalTelephoneValidation,EmailValidation"  // live
			...
			form: {
				...
				validate_form: function validate_form(){ 
					...
					if($("#email").val().length < 3 || !$("#email").val().includes("@")  ){
						return false
					}
				...
				submit_supporting_form_data: function submit_supporting_form_data(){
					...
					let payload = { 
						...
						data: {
							enquiry_id: $("#enquiry_id").val().length ? $("#enquiry_id").val() : "empty",
							company: $("#company").val().length ? $("#company").val() : "empty",
							industry: $("#industry").val().length ? $("#industry").val() : "empty",
							location: $("#location").val().length ? $("#location").val() : "empty",
					...
					app.db_req.send(payload, false).then((response) =>{
						if(response.success){ 
							...
							alert("An error has occured submitting your preperation information, please try again.")
				...
				submit_supporting_form_data: function submit_supporting_form_data(){
					...
					let payload = { 
						...
						data: {
							...
							company: $("#company").val().length ? $("#company").val() : "empty",
							industry: $("#sector").val().length ? $("#sector").val() : "empty",
							location: $("#location").val().length ? $("#location").val() : "empty",
							role: $("#role").val().length ? $("#role").val() : "empty",
							salary: $("#salary").val().length ? $("#salary").val() : "empty"
						}
			...
			form_recruit_first: {  // CURRENTLY IN USE.
				lead_id:"",
				form_data: {
					business: { 
						company: "", 
				...
				validate_form: function validate_form(){ 
					...
					if(!$("#email").hasClass("valid") || !$("#telephone").hasClass("valid")  ){
						return false
					}
				...
				submit_initial_form_data: function submit_initial_form_data(){
					...
					_this.form_data.business.company= $("#company").val().length ? $("#company").val() : "empty",
				...
				submit_supporting_form_data: function submit_supporting_form_data(){
					let _this = this; 
					_this.form_data.business.industry=  $("#sector").val().length ? $("#sector").val() : "empty",
					_this.form_data.business.location=  $("#location").val().length ? $("#location").val() : "empty",
					_this.form_data.business.role=  $("#role").val().length ? $("#role").val() : "empty",
					_this.form_data.business.salary=  $("#salary").val().length ? $("#salary").val() : "empty"
